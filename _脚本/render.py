...
5. **同词多活动**：{F['同词多活动']['词数']} 个搜索词同时被 ≥3 个活动买，合计 {usd(F['同词多活动']['花费'])}＝**{pct(F['同词多活动']['占比'],0)}**。头部 4 词（{'、'.join(F['头部词'])}）花在：{lane_line}；精确组只占 **{pct(F['精确组占头部词花费'])}**。{("关键词竞价 $%s–%s，几乎一刀切。" % tuple(F['活动']['竞价区间'])) if F['活动']['竞价区间'][0] is not None else "（没有提供关键词竞价，跳过竞价判断。）"}
6. **否词没否干净**：{'；'.join(f"{x['词']} 只在「{x['只否在']}」否了，近 14 天仍从另外 {x['别处活动数']} 个活动花 {usd(x['花费'])}（{x['订单']} 单）" for x in lk) or '未发现'}。
7. **库存**：可售 {I['可售']:,}、调拨中 {I['调拨中']:,}、途中 {I['途中']:,}；上周卖 {I['上周销量']:,} 件，整体够 **{I['整体可售周数']:.1f} 周**。将断货：{risk}。两周内会断货的 {len(I['两周内断货SKU'])} 个 SKU（{'、'.join(I['两周内断货SKU'])}）占上周件数 {pct(I['两周内断货占上周件数'],0)}，合计可售 {I['两周内断货可售']} 件＝{(f"{I['两周内断货可售周数']:.2f}" if I['两周内断货可售周数'] is not None else "—")} 周。
